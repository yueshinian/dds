...
class SubListener : public eprosima::fastrtps::SubscriberListener
	...
	{
    		if (sub->takeNextData((void*)&octomap_, &info))
    		{
       			 if (info.sampleKind == ALIVE)
        		{
            			this->n_samples++;
	    			msgAvailale=true;
            			std::cout << "[RTCP] Seq " << octomap_.seq() << " Time" << octomap_.secs() << " RECEIVED" << std::endl;
        		}
    		}
   	}
    ...
    while (ros::ok()) 
    {
		...
		if(false)
		{
			...
			for(int i=0;i<4;i++)
			{
				...
				//ROS_INFO("offset %s",listener.pcl_.PointFileds_name()[i].c_str());
