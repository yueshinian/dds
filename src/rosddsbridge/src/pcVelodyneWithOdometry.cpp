...
public:
    ...
    void on_data_available(eprosima::fastdds::dds::DataReader *reader)
    {
        ...
        if (reader->take_next_sample(&velodyneWithOdom, &info) == ReturnCode_t::RETCODE_OK)
        {
            ...
            {
                if (LOG)
                    std::cout << "[RUDP-PclWithOdom] Seq: " << velodyneWithOdom.pcl().seq() << " with time: " << velodyneWithOdom.pcl().secs() << " RECEIVED" << std::endl;
                ...
                odomTrans.setRotation(tf::Quaternion(odom.pose.pose.orientation.x, odom.pose.pose.orientation.y, odom.pose.pose.orientation.z, odom.pose.pose.orientation.w));
                odomTrans.setOrigin(tf::Vector3(odom.pose.pose.position.x, odom.pose.pose.position.y, odom.pose.pose.position.z));
                ...
                br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "map", "receive_vehicle"));
                */
                if (LOG)
                    ROS_INFO("pub pcl");
            }
...
{
    ros::init(argc, argv, "pcVelodyneWithOdomSub");
    ros::NodeHandle nh;
